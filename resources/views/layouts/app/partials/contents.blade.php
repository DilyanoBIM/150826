...
<div class="md:col-start-2 row-start-2 row-span-3 w-full h-full pt-4 pl-4 md:pt-5 md:pl-5 lg:pt-6 lg:pl-6 transition-all duration-300 min-h-0 {{ $wrapperClass ?? '' }}">
    <div class="bg-white w-full h-full rounded-tl-xl shadow-sm border-t border-l border-slate-200 overflow-hidden flex flex-col">
        ...
        <div class="w-full px-4 py-2.5 md:px-6 lg:px-8 border-b border-slate-200 bg-slate-50/80 flex flex-wrap items-center gap-2 shrink-0">
            ...
            <button type="button" @click="$dispatch('toolbar-add')" {!! $addAttributes ?? '' !!} class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white text-xs font-semibold rounded-lg shadow-sm transition-colors cursor-pointer">
            ...
            <button type="button" @click="$dispatch('toolbar-save')" {!! $saveAttributes ?? '' !!} class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white text-xs font-semibold rounded-lg shadow-sm transition-colors cursor-pointer">
            ...
            <button type="button" @click="$dispatch('toolbar-show')" {!! $showAttributes ?? '' !!} class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white hover:bg-sky-50 active:bg-sky-100 text-sky-700 border border-sky-200 hover:border-sky-300 text-xs font-medium rounded-lg transition-colors cursor-pointer">
            ...
            <button type="button" @click="$dispatch('toolbar-edit')" {!! $editAttributes ?? '' !!} class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white hover:bg-amber-50 active:bg-amber-100 text-amber-700 border border-amber-200 hover:border-amber-300 text-xs font-medium rounded-lg transition-colors cursor-pointer">
            ...
            <button type="button" @click="$dispatch('toolbar-duplicate')" {!! $duplicateAttributes ?? '' !!} class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white hover:bg-slate-100 active:bg-slate-200 text-slate-700 border border-slate-200 text-xs font-medium rounded-lg transition-colors cursor-pointer">
            ...
            <button type="button" @click="$dispatch('toolbar-archive')" {!! $archiveAttributes ?? '' !!} class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 text-xs font-medium rounded-lg transition-colors cursor-pointer">
                <svg class="w-3.5 h-3.5 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4"></path></svg>
                {{ $archiveText ?? 'Arsipkan' }}
            </button>
            @endif

            @if($showDeleteButton ?? true)
            <button type="button" @click="$dispatch('toolbar-delete')" {!! $deleteAttributes ?? '' !!} class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white hover:bg-rose-50 active:bg-rose-100 text-rose-600 border border-rose-200 hover:border-rose-300 text-xs font-medium rounded-lg transition-colors cursor-pointer">
            ...
            <div class="inline-flex rounded-lg border border-slate-200 bg-white p-0.5 shadow-sm">
                <button type="button" @click="$dispatch('toolbar-undo')" {!! $undoAttributes ?? '' !!} title="Undo (Urungkan)" class="p-1 text-slate-600 hover:bg-slate-100 rounded transition-colors cursor-pointer">
                ...
                <button type="button" @click="$dispatch('toolbar-redo')" {!! $redoAttributes ?? '' !!} title="Redo (Ulangi)" class="p-1 text-slate-600 hover:bg-slate-100 rounded transition-colors cursor-pointer">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 10H11a5 5 0 00-5 5v2m15-7l-4-4m4 4l-4 4"></path></svg>
                </button>
            </div>
            @endif

            @if($showHistoryButton ?? true)
            <button type="button" @click="$dispatch('toolbar-history')" {!! $historyAttributes ?? '' !!} title="Log Aktivitas & Riwayat Perubahan" class="inline-flex items-center gap-1.5 px-2.5 py-1.5 bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 text-xs font-medium rounded-lg transition-colors cursor-pointer">
            ...
            <button type="button" @click="$dispatch('toolbar-date-filter')" {!! $dateFilterAttributes ?? '' !!} class="inline-flex items-center gap-1.5 px-2.5 py-1.5 bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 text-xs font-medium rounded-lg transition-colors cursor-pointer">
            ...
            <button type="button" @click="$dispatch('toolbar-filter')" {!! $filterAttributes ?? '' !!} class="inline-flex items-center gap-1.5 px-2.5 py-1.5 bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 text-xs font-medium rounded-lg transition-colors cursor-pointer">
            ...
            <div class="inline-flex rounded-lg border border-slate-200 bg-white p-0.5 shadow-sm">
                <button type="button" @click="$dispatch('toolbar-view', 'table')" title="Tampilan Tabel" class="p-1 bg-slate-100 text-blue-600 rounded transition-colors cursor-pointer">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 10h16M4 14h16M4 18h16"></path></svg>
                </button>
                <button type="button" @click="$dispatch('toolbar-view', 'grid')" title="Tampilan Kartu / Grid" class="p-1 text-slate-500 hover:text-slate-700 hover:bg-slate-50 rounded transition-colors cursor-pointer">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"></path></svg>
                </button>
                <button type="button" @click="$dispatch('toolbar-view', 'board')" title="Tampilan Kolom / Board" class="p-1 text-slate-500 hover:text-slate-700 hover:bg-slate-50 rounded transition-colors cursor-pointer">
            ...
            <button type="button" @click="$dispatch('toolbar-columns')" {!! $columnAttributes ?? '' !!} title="Kustomisasi Tampilan Kolom" class="inline-flex items-center gap-1.5 px-2.5 py-1.5 bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 text-xs font-medium rounded-lg transition-colors cursor-pointer">
                <svg class="w-3.5 h-3.5 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17V7m0 10a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2h2a2 2 0 012 2m0 10a2 2 0 002 2h2a2 2 0 002-2V7a2 2 0 00-2-2h-2a2 2 0 00-2 2m0 10V7m0 10a2 2 0 002 2h2a2 2 0 002-2V7a2 2 0 00-2-2h-2a2 2 0 00-2 2"></path></svg>
